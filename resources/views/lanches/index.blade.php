...
@section('breadcrumb')
    <li class="breadcrumb-item"><span style="color:#94a3b8;">Cardápio</span></li>
    <li class="breadcrumb-item active">Lanches</li>
@endsection
...
<div class="page-header">
    <div>
        <h1 class="page-title">🍔 Lanches</h1>
        <p class="page-subtitle">Gerencie todos os lanches do cardápio</p>
    </div>
    <a href="{{ route('lanches.create') }}" class="btn btn-primary">
        <i class="bi bi-plus-lg me-1"></i> Novo Lanche
    </a>
</div>
...
<div class="card">
    <div class="card-header">
        ...
        <div class="d-flex gap-2 align-items-center flex-wrap">
            <form method="GET" action="{{ route('lanches.index') }}" class="d-flex gap-2">
                <input
                    type="search"
                    name="busca"
                    value="{{ request('busca') }}"
                ...
                <button type="submit" class="btn btn-primary">
                    ...
                    <a href="{{ route('lanches.index') }}" class="btn"
    ...
    <div class="table-responsive">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td style="color:#94a3b8; font-size:.8rem;">{{ $lanche->id }}</td>
                    <td>
                        <div style="font-weight:600; color:#1e293b;">{{ $lanche->nome }}</div>
                        @if($lanche->descricao)
                            <div style="font-size:.75rem; color:#94a3b8;">
                                {{ Str::limit($lanche->descricao, 45) }}
                            </div>
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        <div class="d-flex justify-content-center gap-1">
                            ...
                            <a 
                               ...
                               title="Visualizar"
                               onclick="visualizarLanche({{ $lanche->id }})">
                                <i class="bi bi-eye"></i>
                            </a>

                            
                            <!-- Editar -->
                            <a href="{{ route('lanches.edit', $lanche) }}"
                            ...
                            <button type="button"
                                    ...
                                    title="Excluir"
                                    onclick="confirmarExclusao({{ $lanche->id }}, '{{ addslashes($lanche->nome) }}')">
                                <i class="bi bi-trash3"></i>
                            </button>

                            <!-- Form oculto de exclusão -->
                            <form id="form-delete-{{ $lanche->id }}"
                                  action="{{ route('lanches.destroy', $lanche) }}"
                ...
                <tr>
                    <td colspan="7" class="text-center py-5">
                        ...
                        <div style="font-size:.82rem; color:#94a3b8; margin-bottom:16px;">
                            ...
                            @else
                                Comece cadastrando o primeiro lanche do cardápio.
                            @endif
                        </div>
                        <a href="{{ route('lanches.create') }}" class="btn btn-primary btn-sm">
                            <i class="bi bi-plus-lg me-1"></i> Cadastrar lanche
                        </a>
                    </td>
                </tr>
                @endforelse
    ...
    <div class="d-flex align-items-center justify-content-between px-4 py-3"
        ...
        <div style="font-size:.8rem; color:#94a3b8;">
            Exibindo {{ $lanches->firstItem() }}–{{ $lanches->lastItem() }}
            de {{ $lanches->total() }} registros
        </div>
        {{ $lanches->withQueryString()->links() }}
    </div>
    @endif
</div>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px;">
        <div class="modal-content border-0 shadow-lg" style="border-radius:16px; overflow:hidden;">
            <div class="modal-body p-4 text-center">
                <div style="font-size:2.5rem; margin-bottom:12px;">🗑️</div>
                <h5 style="font-weight:700; color:#1e293b; margin-bottom:8px;">Excluir lanche?</h5>
...
<div class="modal fade" id="modalVisualizar" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px;">
        <div class="modal-content border-0 shadow-lg" style="border-radius:16px; overflow:hidden;">
            <div class="modal-body p-4 text-center">
                <div style="font-size:2.5rem; margin-bottom:12px;">👁️</div>
                <h5 style="font-weight:700; color:#1e293b; margin-bottom:8px;">Visualizar lanche</h5>
                <p class="text-muted mb-0" style="font-size:.875rem;">
                    <strong id="nomeVisualizar"></strong><br>
                    <span id="descricaoVisualizar"></span>
                </p>
            </div>
            <div class="modal-footer border-0 pt-0 pb-4 px-4 d-flex gap-2">
                <button type="button" class="btn flex-fill"
...
<script>
function visualizarLanche(id) {
    fetch(`/lanches/${id}`)
        .then(response => response.json())
        .then(data => {

            document.getElementById('nomeVisualizar').textContent = data.nome;
            document.getElementById('descricaoVisualizar').textContent = data.descricao;
